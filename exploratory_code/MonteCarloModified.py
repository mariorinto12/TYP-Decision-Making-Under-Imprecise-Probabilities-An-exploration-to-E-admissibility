...
# Establishment of three different portfolios with different risk profiles (weights)
portfolios = {
    ...
    "Aggressive": { # Higher weight on high annual mean return assets
        "BBVA": 0.20, "ACS": 0.15, "Repsol": 0.20, "Amadeus": 0.15,
        "Inditex": 0.10, "Telefonica": 0.10, "Iberdrola": 0.00,
        "Gold": 0.00, "Bonds": 0.00
    },
    
    "MarketIndex": { # Weights similar to a market index (e.g., IBEX 35)
        "BBVA": 0.18, "ACS": 0.12, "Repsol": 0.14, "Amadeus": 0.10,
        "Inditex": 0.20, "Telefonica": 0.10, "Iberdrola": 0.10,
        "Gold": 0.03, "Bonds": 0.03
    },
    
    "GoldHedge": { # Higher weight on gold as a hedge against market downturns
        "BBVA": 0.05, "ACS": 0.05, "Repsol": 0.10, "Amadeus": 0.10,
        "Inditex": 0.10, "Telefonica": 0.10, "Iberdrola": 0.10,
        "Gold": 0.30, "Bonds": 0.10
    },
    
    "BondHeavy": { # Higher weight on bonds for income and stability
        "BBVA": 0.00, "ACS": 0.00, "Repsol": 0.00, "Amadeus": 0.05,
        "Inditex": 0.05, "Telefonica": 0.10, "Iberdrola": 0.10,
        "Gold": 0.10, "Bonds": 0.60
    },
    
    "EnergyBoost": { # Higher weight on energy sector (Repsol, Iberdrola) for potential growth
        "BBVA": 0.00, "ACS": 0.10, "Repsol": 0.40, "Amadeus": 0.00,
        "Inditex": 0.10, "Telefonica": 0.00, "Iberdrola": 0.30,
        "Gold": 0.05, "Bonds": 0.05
    },

    "TechTravel": { # Higher weight on technology, travel and consumer-dependent (Amadeus, Inditex) for potential growth
        "BBVA": 0.05, "ACS": 0.05, "Repsol": 0.05, "Amadeus": 0.40,
        "Inditex": 0.30, "Telefonica": 0.10, "Iberdrola": 0.05,
        "Gold": 0.00, "Bonds": 0.00
    },
    
    "DefensiveEquity": { # Higher weight on defensive stocks (Iberdrola, Telefonica) and gold for stability
        "BBVA": 0.00, "ACS": 0.10, "Repsol": 0.05, "Amadeus": 0.10,
        "Inditex": 0.30, "Telefonica": 0.10, "Iberdrola": 0.30,
        "Gold": 0.03, "Bonds": 0.02
    },
    
    "AntiCrisis": { # Higher weight on gold and bonds as a hedge against market downturns, minimal exposure to stocks
        "BBVA": 0.00, "ACS": 0.00, "Repsol": 0.00, "Amadeus": 0.05,
...
def compute_f_definitions(
    ...
    for c in range(num_clusters):
        ...
        f3_signed_mean = float(np.mean([signed_score(x) for x in center]))
        ...
        f_per_cluster.append({
            ...
            "f3_signed_score": round(f3_signed_mean, 4),
...
def demo():
    ...
    # If you want to map clusters to theta_1..theta_5 explicitly:
